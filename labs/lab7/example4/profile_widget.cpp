...
ProfileWidget::ProfileWidget(std::string name, std::string email, std::string website) {
	...
	connect(addFriendButton, SIGNAL(clicked()), this, SLOT(addFriend())); //addFriendButton emits signal when clicked, sends it back to us and we call method addFriend()
...
{
	QMessageBox::information(this, "Add Friend" , "We're now friends!");
}
